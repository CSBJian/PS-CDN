...
with torch.no_grad():
    results = compute_all_metrics(all_recons.to(args.device), all_ref.to(args.device), args.batch_size)
    results = {k:v.item() for k, v in results.items()}
    jsd = jsd_between_point_cloud_sets(all_recons.cpu().numpy(), all_ref.cpu().numpy())
    results['jsd'] = jsd
