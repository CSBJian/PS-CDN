...
class VAE(Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.encoder = VMFPointNetEncoder(args)
        self.diffusion = DiffusionPoint(
            net = PointwiseNetAtten(point_dim=3, context_dim=args.latent_dim, residual=args.residual),
...
class VMFPointNetEncoder(Module):

    def __init__(self, args):
        super().__init__()
        self.args = args
        self.encoder = Encoder(zdim=args.latent_dim)
        self.diffusion = DiffusionPoint(
            net = PointwiseNet(point_dim=3, context_dim=args.latent_dim, residual=args.residual),
            var_sched = VarianceSchedule(
                num_steps=args.num_steps,
