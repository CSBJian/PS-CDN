...
def emd_approx(x, y):
    ...
    emd = emd_cuda.matchcost_forward(xyz1, xyz2, match)
    emd = emd / npts
    return emd


# def emd_approx(x, y):
#     emd = torch.zeros([x.size(0)]).to(x)
#     return emd
...
def compute_Nll_metrics(sample_pcs, ref_pcs, batch_size):
    results = {}
...
def compute_all_metrics_iter(sample_pcs, ref_pcs, batch_size, M_rr_cd, M_rr_emd):
    results = {}
...
if __name__ == '__main__':
    ...
    results = compute_all_metrics(a, b, batch_size=16)
