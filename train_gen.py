...
# Model arguments
parser.add_argument('--model', type=str, default='gaussian', choices=['flow', 'gaussian', 'vmf'])
parser.add_argument('--backbone', type=str, default='pw', choices=['pw', 'sa'])
...
parser.add_argument('--sched_start_epoch', type=int, default=300000)
parser.add_argument('--sched_end_epoch', type=int, default=800000)
...
parser.add_argument('--max_iters', type=int, default=float('1000000'))
...
# Logging
if args.logging:
    log_dir = get_new_log_dir(args.log_root, prefix='GEN_'+ args.categories[0] + args.model + args.backbone + str(args.latent_dim) +'_', postfix='_' + args.tag if args.tag is not None else '')
...
if args.model == 'gaussian':
    ...
    model = VMFVAE(args).to(args.device)
...
if len(ckpt_mgr.ckpts) >= 1:
    ckpt_dir = ckpt_mgr.save_dir + "\\" + ckpt_mgr.ckpts[len(ckpt_mgr.ckpts)-1]['file']
    # 检测继续训练
    ckpt = torch.load(ckpt_dir)
    model.load_state_dict(ckpt['state_dict'])
    optimizer.load_state_dict(ckpt['others']['optimizer'])
    scheduler.load_state_dict(ckpt['others']['scheduler'])
    iteration = ckpt_mgr.ckpts[len(ckpt_mgr.ckpts)-1]['iteration']+1
else:
    iteration = 1
...
def train(it):
    ...
    # b = batch['pointcloud'][0].numpy()
    # np.savetxt("point.csv", b)
...
def validate_inspect(it):
    if args.model == 'gaussian':
        ...
        p_z = HypersphericalUniform(args.latent_dim - 1)
        z = p_z.sample(shape=torch.tensor([args.val_batch_size, ])).squeeze(1).to(args.device)
